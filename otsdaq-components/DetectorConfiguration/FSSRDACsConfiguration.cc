...
FSSRDACsConfiguration::FSSRDACsConfiguration(void) {
  ...
  //      <COLUMN Name="DigContrReg"   StorageNanameToROCDACsMap_me="DIG_CONTR_REG"  DataType="NUMBER"   />
  //      <COLUMN Name="AqBCO"         StorageName="AQ_BCO"         DataType="NUMBER"   />
  //    </VIEW>
  //  </CONFIGURATION>
  //</ROOT>
...
//==============================================================================
const ROCDACs FSSRDACsConfiguration::getROCDACs(const ConfigurationTree& dacsConfiguration) {
  ROCDACs rocDACs;
  for (auto& it : dacNameToDACAddress_) {
    ...
    // dacsConfiguration.getNode(it.first).getValue<unsigned int>() << " size: " << rocDACs.getDACList().size()<<
    // std::endl;
  }
