...
void DACStream::makeStream(const ConfigurationTree& feToDetectorTree) {
  ...
  for (auto& it : feWriterDetectorList) {
    ...
    //__COUT__ << dacsConfiguration << std::endl;
    currentElement = theChannelStreamMap_.insert(
        std::pair<unsigned int, ROCStream>(it.second.getNode("FEWriterChannel").getValue<unsigned int>(), ROCStream()));
    ...
    currentElement->second.setROCStatus(
        detectorConfiguration.getNode(ViewColumnInfo::COL_NAME_STATUS).getValue<bool>());
