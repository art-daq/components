...
//==============================================================================
DACsConfigurationBase::DACsConfigurationBase(std::string configurationName, unsigned int rocNameColumn, unsigned int firstDAC, unsigned int lastDAC) :
...
const ROCDACs& DACsConfigurationBase::getROCDACs(std::string rocName) const
{
	...
	//
	//for(ots::DACList::const_iterator it = nameToROCDACsMap_.find(rocName)->second.getDACList().begin(); it != nameToROCDACsMap_.find(rocName)->second.getDACList().end(); ++it)
	//	std::cout << __COUT_HDR_FL__ << "DAC Name :" << it->first << ", value: " << it->second.second << std::endl;
