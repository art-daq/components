...
class FrontEndFirmwareBase {
 public:
  ...
  // {__SS__; __THROW__(ss.str() + "Illegal call to undefined base class member function"); return "";};
  virtual uint32_t createRegisterFromValue(std::string& readBuffer, std::string& receivedValue) {
    __SS__;
    __THROW__(ss.str() + "Illegal call to undefined base class member function");
    return 0;
  };
  // virtual uint64_t     createRegisterFromValue     (std::string& readBuffer, std::string& receivedValue)
  // {__SS__; __THROW__(ss.str() + "Illegal call to undefined base class member function"); return 0;};

  // purdue firmware
  //    virtual std::string  setDataDestination          (std::string ip, unsigned int port);
  //    virtual unsigned int waitSet                     (std::string& buffer, unsigned int address, unsigned int data,
  //    unsigned int timeout = 255); virtual unsigned int waitClear                   (std::string& buffer, unsigned int
  //    address, unsigned int data, unsigned int timeout = 255); virtual unsigned int getNumberOfBufferedCommands
  //    (std::string& buffer); virtual std::string  compareSendAndReceive       (const std::string& sentBuffer,
  //    std::string& acknowledgment); virtual uint32_t     createRegisterFromValue     (std::string& readBuffer,
  //    std::string& receivedValue);

  // FSSROtsFirmware
  //    virtual void makeDACSequence 				(FirmwareSequence<uint64_t>& sequence, unsigned int
  //    channel, const ROCStream& rocStream);
  //    virtual void makeMaskSequence				(FirmwareSequence<uint64_t>& sequence, unsigned int
  //    channel, const ROCStream& rocStream);
  //    virtual void makeDACSequence 				(FirmwareSequence<uint32_t>& sequence, unsigned int
  //    channel, const ROCStream& rocStream);
  //    virtual void makeDACBuffer   				(std::string& buffer, unsigned int channel, const
  //    ROCStream& rocStream);
  //    virtual void makeDACBuffer   				(std::vector<std::string>& buffer, unsigned int
  //    channel, const ROCStream& rocStream);
  //    virtual void makeMaskSequence				(FirmwareSequence<uint32_t>& sequence, unsigned int
  //    channel, const ROCStream& rocStream);
  //    virtual void makeMaskBuffer  				(std::string& buffer, unsigned int channel, const
  //    ROCStream& rocStream);
  ...
  //    	return "";
  //    };
  //    virtual void makeDACBuffer(std::vector<std::string>& buffer, unsigned int channel, const ROCStream&
  //    rocStream){__SS__; __THROW__(ss.str() + "Illegal call to undefined base class member function");  return;};
  //    virtual void makeMaskBuffer(std::string& buffer, unsigned int channel, const ROCStream& rocStream){__SS__;
  //    __THROW__(ss.str() + "Illegal call to undefined base class member function");  return;};
