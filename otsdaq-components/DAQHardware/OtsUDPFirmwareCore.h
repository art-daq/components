...
class OtsUDPFirmwareCore : public FrontEndFirmwareBase {
 ...
 public:
  ...
  void writeAdvanced(std::string& buffer, const char* address, const char* data, uint8_t size = 1,
                     uint8_t commandTypeOptions = 0, bool clearBuffer = true);
  void writeAdvanced(std::string& buffer, const uint64_t& address, const char* data, uint8_t size,
                     uint8_t commandTypeOptions = 0,
                     bool clearBuffer = true);  // size is required, to remove ambiguity when calling write with data=0
  void writeAdvanced(std::string& buffer, const uint64_t& address, const uint64_t& data, uint8_t commandTypeOptions = 0,
                     bool clearBuffer = true);
  void writeAdvanced(std::string& buffer, const uint64_t& address, const std::vector<uint64_t>& data,
                     uint8_t commandTypeOptions = 0, bool clearBuffer = true);
  ...
  void setControlDestinationAsSelf(std::string& buffer) {
    buffer.resize(0);
    buffer += (char)1; /* special one-byte packet, sets control destination as sender */
  }
  ...
  void UDPFirmwareSpecialNoOp(std::string& buffer) {
    buffer.resize(0);
    buffer += (char)3; /* special one-byte packet, is ignored at OIE - could be used to keep connection alive */
  }
