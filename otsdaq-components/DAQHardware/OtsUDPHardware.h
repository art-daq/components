...
class OtsUDPHardware : public TransceiverSocket, public FrontEndHardwareBase {
 public:
  OtsUDPHardware(std::string hostIPAddress, unsigned int hostPort, std::string boardIPAddress, unsigned int boardPort,
  ...
  virtual void read(const std::string& sendBuffer, std::string& receiveBuffer,
                    int timeoutSeconds = -1) throw(std::runtime_error);
  virtual void read(const std::vector<std::string>& sendBuffers, std::vector<std::string>& receiveBuffers,
                    int timeoutSeconds = -1) throw(std::runtime_error);

  virtual int flushRead() throw(std::runtime_error) { return clearReadSocket(); }

  void read(const std::string& sendBuffer, uint64_t& receiveQuadWord,
            int timeoutSeconds = -1) throw(std::runtime_error);
  void read(const std::string& sendBuffer, std::vector<uint64_t>& receiveQuadWords,
            int timeoutSeconds = -1) throw(std::runtime_error);
  void read(const std::vector<std::string>& sendBuffers, std::vector<std::vector<uint64_t> >& receiveQuadWordsVector,
            int timeoutSeconds = -1) throw(std::runtime_error);
  int clearReadSocket();
  //
