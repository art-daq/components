...
//========================================================================================================================
OtsUDPHardware::OtsUDPHardware(std::string hostIPAddress, unsigned int hostPort, std::string OtsUDPHardwareIPAddress,
                               unsigned int OtsUDPHardwarePort, unsigned int version, bool verbose)
  ...
  __COUT__ << "Socket for hardware is initialized at IP:Port " << OtsUDPBoard_.getIPAddress() << ":"
...
void OtsUDPHardware::write(const std::string& sendBuffer) throw(std::runtime_error) try {
  ...
  //			ss << std::setfill('0') << std::setw(2) << std::hex << (((int16_t) sendBuffer[i]) &0xFF) << "-" <<
  //std::dec;
  //		}
  //		ss << std::endl;
  ...
  if (TransceiverSocket::send(OtsUDPBoard_, sendBuffer, verbose_) < 0) {
    __SS__ << "Write failed to hardware at IP:Port " << OtsUDPBoard_.getIPAddress() << ":" << OtsUDPBoard_.getPort()
           << __E__;
    __SS_THROW__;
  }
...
void OtsUDPHardware::write(const std::vector<std::string>& sendBuffer) throw(std::runtime_error) {
  for (const auto& it : sendBuffer) {
    ...
    //				if(i==begin+2) ss << ":::";
    //				else if(i==begin+10) ss << ":::";
    //				ss << std::setfill('0') << std::setw(2) << std::hex << (((const int16_t) it[i]) &0xFF) << "-" <<
    //std::dec;
    //			}
    //			ss << std::endl;
    //			fprintf(fp,"%s",ss.str().c_str());
...
//========================================================================================================================
void OtsUDPHardware::writeAndAcknowledge(const std::string& buffer, int timeoutSeconds) throw(std::runtime_error) try {
  //	char msg[100];
  //	sprintf(msg,"_%d",getPort());
  //	std::string fn = "/tmp/new_udp_chk" + std::string(msg) + ".dat";
  //	FILE *fp = fopen(fn.c_str(),"a");
  ...
  //			if(i==begin+2) ss << ":::";
  //			else if(i==begin+10) ss << ":::";
  //			ss << std::setfill('0') << std::setw(2) << std::hex << (((int16_t) buffer[i]) &0xFF) << "-" <<
  //std::dec;
  //		}
  //		ss << std::endl;
  //		fprintf(fp,"%s",ss.str().c_str());
  ...
  if (TransceiverSocket::receive(acknowledgment_, timeoutSeconds) < 0) {
    __SS__ << "writeAndAcknowledge failed from hardware at IP:Port " << OtsUDPBoard_.getIPAddress() << ":"
           << OtsUDPBoard_.getPort() << ". Timeout period of " << timeoutSeconds << " seconds reached without response."
           << std::endl;
    __COUT_ERR__ << "\n" << ss.str() << std::endl;
    __SS_THROW__;
  }
  ...
  // Check First Byte:
  // (https://docs.google.com/document/d/1i3Z07n8Jq78NwgUFdjAv2sLGhH4rWjHeYEScAWBzSyw/edit?usp=sharing) 	bits 2:0 =
  //Packet Type (0: Read, 1: First in Burst, 2: Burst Middle, 3: Last in Burst, 4: Read Ack, 5: Write Ack, 6: Burst
  //Start Ack, 7: Burst Stop Ack) 	bit 3 = ack of no address increment op 	bit 4 = err detected in protocol since
  //reset 	bit 5 = rx overflow since reset 	bit 6 = crc err flag 	bit 7 = crc err detected
...
void OtsUDPHardware::read(const std::string& sendBuffer, std::string& receiveBuffer,
  ...
  //	std::cout << std::setfill('0') << std::setw(2) << std::hex << (((int16_t) receiveBuffer[i]) &0xff) << "-" <<
  //std::dec;  std::cout << std::endl;
...
//========================================================================================================================
void OtsUDPHardware::read(const std::vector<std::string>& sendBuffers, std::vector<std::string>& receiveBuffers,
                          int timeoutSeconds) throw(std::runtime_error) {
  receiveBuffers.resize(sendBuffers.size());
  auto receiveBufferIterator = receiveBuffers.begin();
  for (const auto& sendBuffer : sendBuffers) read(sendBuffer, *(receiveBufferIterator++));
}

//========================================================================================================================
void OtsUDPHardware::read(const std::string& sendBuffer, uint64_t& receiveQuadWord,
                          int timeoutSeconds) throw(std::runtime_error) {
  ...
  if (receiveBuffer.length() != 10) {
    __SS__ << "Read uint64_t quad-word failed. Invalid size of received buffer: " << receiveBuffer.length()
           << " != " << 10 << std::endl;
    __SS_THROW__;
  }
...
//========================================================================================================================
void OtsUDPHardware::read(const std::string& sendBuffer, std::vector<uint64_t>& receiveQuadWords,
                          int timeoutSeconds) throw(std::runtime_error) {
  ...
  if ((receiveBuffer.length() - 2) % 8 != 0) {
    __SS__ << "Read vector of uint64_t quad-word failed. Invalid size of received buffer: (" << receiveBuffer.length()
           << " - 2) % 8 != 0" << std::endl;
    __SS_THROW__;
  }
...
int OtsUDPHardware::clearReadSocket() {
  ...
  try {
    while (TransceiverSocket::receive(dummerReceiveBuffer, 0 /*timeoutSeconds*/, 0 /*timeoutUSeconds*/,
                                      false /*verbose*/) >= 0)
      ++cnt;
  } catch (...) {
