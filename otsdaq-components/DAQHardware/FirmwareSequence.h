...
class FirmwareSequence {
 public:
  typedef std::vector<std::pair<T, std::vector<T> > >
      Sequence;  // It is a vector of pairs of addresses and data to send to the Firmware
  ...
  inline void pushBack(T address, const T& data) {
    theSequence_.push_back(std::pair<T, std::vector<T> >(address, std::vector<T>(1, data)));
  }
