...
{
public:
	typedef std::vector<std::pair<T,std::vector<T> > > Sequence;//It is a vector of pairs of addresses and data to send to the Firmware
    ...
    inline void pushBack(std::pair<T,std::vector<T> > entry)    {theSequence_.push_back(entry);}
    inline void pushBack(T address, const std::vector<T>& data) {theSequence_.push_back(std::pair<T,std::vector<T> >(address, data));}
    inline void pushBack(T address, const T& data)              {theSequence_.push_back(std::pair<T,std::vector<T> >(address, std::vector<T>(1,data)));}
