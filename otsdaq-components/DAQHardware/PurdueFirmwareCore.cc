...
//========================================================================================================================
PurdueFirmwareCore::PurdueFirmwareCore(unsigned int version) : FrontEndFirmwareBase(version) {}
...
//========================================================================================================================
void PurdueFirmwareCore::setDataDestination(std::string& buffer, const std::string& ipAddress, const uint16_t port,
                                            bool clearBuffer) {
  // do nothing
    ...
    for (int position = 0; position < 4; position++) {
      ...
      if (position == 0) {
        if (sentBuffer[position + skipBuffer] != acknowledgment[position + skipAcknow]) {
          std::cout << __COUT_HDR_FL__ << "ERROR - Sent " << position + skipBuffer << "th: " << std::hex
                    << (unsigned int)sentBuffer[position + skipBuffer] << std::dec << " different from Received "
                    << position + skipAcknow << "th:" << std::hex << (unsigned int)acknowledgment[position + skipAcknow]
                    << std::dec;
      ...
      if (position == 3) {
        ...
        if (((remainBufferCommands == 12) && (remainAcknowCommands == 4)) ||
          ...
          std::cout << __COUT_HDR_FL__ << "ERROR - Sent " << position + skipBuffer << "th: " << std::hex
                    << (unsigned int)sentBuffer[position + skipBuffer] << std::dec << " not compatible with Received "
                    << position + skipAcknow << "th:" << std::hex << (unsigned int)acknowledgment[position + skipAcknow]
                    << std::dec << std::endl;
          std::cout << __COUT_HDR_FL__ << "Position: " << position << std::endl;
          reSendBuffer.clear();
...
uint32_t PurdueFirmwareCore::createRegisterFromValue(std::string& readBuffer, std::string& receivedValue) {
  for (int position = 0; position < 8; position++) {
    if (position == 0 || position == 4 || position == 5 || position == 6 || position == 7) {
      if (readBuffer[position] != receivedValue[position]) {
        std::cout << __COUT_HDR_FL__ << "ERROR - SentBuffer position " << position << " value: " << std::hex
                  << (unsigned int)readBuffer[position] << std::dec << " different from ReceivedBuffer position "
                  << position << " value: " << std::hex << (unsigned int)receivedValue[position] << std::dec
                  << std::endl;
        return 0;
      }
    }
  ...
  registerValue = ((unsigned int)receivedValue[8] << 24) + ((unsigned int)receivedValue[9] << 16) +
