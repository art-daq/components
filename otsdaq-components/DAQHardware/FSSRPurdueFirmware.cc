...
//========================================================================================================================
FSSRPurdueFirmware::FSSRPurdueFirmware(std::string communicationFirmwareType, unsigned int communicationFirmwareVersion,
...
//	//protocolInstance_ = new OtsFirmwareCore(version);//AUG-17-2017 RAR dissociated because function calls are
//entirely independent from PURDUE firmware calls // //FrontEndFirmwareBase::getInstance(type,version);
...
std::string FSSRPurdueFirmware::configureClocks(std::string source, double frequency)
{
        ...
        OtsUDPFirmwareCore::write(buffer, STRIP_CSR, stripCSRRegisterValue_); //  Reset CSR - reset trigger counter,
external 27 MHz clock
        ...
        ;
        //FIXME? Does Nothing RAR Aug 2017// OtsUDPFirmwareCore::waitClear(buffer, STRIP_CSR, waitDCMResetStripCSR());
//  Wait for DCM to lock
        ...
        //FIXME? Does Nothing RAR Aug 2017//OtsUDPFirmwareCore::waitClear(buffer, STRIP_CSR, waitDCMResetStripCSR()); //
Wait for DCM to lock std::cout << __COUT_HDR_FL__ << "stripCSRRegisterValue :" << std::hex << stripCSRRegisterValue_ <<
std::dec << std::endl;
...
std::string FSSRPurdueFirmware::resetDetector(int channel)
{
        ...
        if (channel == -1)//reset all channels
        {
                ...
                //FIXME? Does Nothing RAR Aug 2017//OtsUDPFirmwareCore::waitClear(buffer, STRIP_RESET, 0xf0000000); //
Wait for reset to complete // was 0xf0000000 } else
        {
...
BIT IS UNSET sendTriggerDataStripCSR(true); sendTriggerNumberStripCSR(true); sendBCOStripCSR(true);
        ...
        BCOOffset(4);
        OtsUDPFirmwareCore::write(buffer, STRIP_TRIG_CSR, stripTriggerCSRRegisterValue_); //  BCO offset // was
0x00000004

        //  write(buffer,STRIP_TRIG_INPUT_0,0x1f060040);  //  FSSR GOTHIT trigger input - timed in for the 27 MHz
external clock
        ...
        if (OtsUDPFirmwareCore::version_ == 1)
                OtsUDPFirmwareCore::write(buffer, STRIP_TRIG_INPUT_3, 0x20401000); // Turn on streaming hits along with
BCO data else if (OtsUDPFirmwareCore::version_ == 2) OtsUDPFirmwareCore::write(buffer, STRIP_TRIG_INPUT_3, 0x20301000);
// Turn on streaming hits along with BCO data else
        {
...
//========================================================================================================================
void FSSRPurdueFirmware::writeBurstDestinationIP(std::string& buffer, const uint64_t value)
{
        OtsUDPFirmwareCore::write(buffer, (uint64_t)0x0000000100000006, value);
}

//========================================================================================================================
void FSSRPurdueFirmware::writeBurstDestinationMAC(std::string& buffer, const uint64_t value)
{
        OtsUDPFirmwareCore::write(buffer, (uint64_t)0x0000000100000007, value);
}

//========================================================================================================================
void FSSRPurdueFirmware::writeBurstDestinationPort(std::string& buffer, const uint64_t value)
{
...
//========================================================================================================================
std::string FSSRPurdueFirmware::startStream(bool channel0, bool channel1, bool channel2, bool channel3, bool channel4,
bool channel5)
{
                ...
                unsigned int channel, const ROCStream& rocStream)
{
        ...
        != rocDACs.getDACList().end(); it++)
        {
                 //RYAN
         //if(it->first != "SendData" && it->first != "RejectHits") continue;
         uint64_t data = FSSRROCDefinitions::makeDACWriteCommand(rocStream.getFEWROCAddress(), it->first,
it->second.second);
         //sequence.pushBack(ChannelFIFOAddress[channel], data);
                ...
                unsigned int channel, const ROCStream& rocStream)
{
        ...
        for (DACList::const_iterator it = rocDACs.getDACList().begin(); it != rocDACs.getDACList().end(); it++)
        {
                ...
                //FIXME? Does Nothing RAR Aug 2017//OtsUDPFirmwareCore::waitClear(bufferElement, STRIP_SC_CSR,
...
//========================================================================================================================
void FSSRPurdueFirmware:: makeDACBuffer(std::vector<std::string>& buffer, unsigned int channel, const ROCStream&
rocStream)
{
...
//========================================================================================================================
void FSSRPurdueFirmware::makeMaskBuffer(std::string& buffer, unsigned int channel, const ROCStream& rocStream, const
std::string& registerName)
{
        ...
        { //d goes from 0 to 4. 4 bytes
                ...
                { //m goes from 0 to 31, since each byte has 8 bits, there are 32 bits
                        val = mask[(8 * 4 * d) + m]; //assigns to val the value of the corresponding bit. 0-31, 32-63,
64-95, 96-127. it goes through each of the 128 bits
                        //std::cout << __COUT_HDR_FL__ << "---------------------" << j++ << std::endl;
                        //std::cout << __COUT_HDR_FL__ << "data[" << d << "] before: " << std::hex << data[d] <<
std::dec << std::endl; data[d] |= (unsigned int) atoi(&val) << (8 * 4 - 1 - m);
                        //std::cout << __COUT_HDR_FL__ << "(unsigned int) atoi(&val): " << (unsigned int) atoi(&val) <<
std::endl;
                        //std::cout << __COUT_HDR_FL__ << "data[" << d << "] after: " << std::hex << data[d] << std::dec
<< std::endl;
                        //std::cout << __COUT_HDR_FL__ << std::hex << "D: " << data[d] << " Val: " << (unsigned
int)atoi(&val) << " index: " << std::dec << (8*4-1-m) << " bit: " << mask[(8*4*d)+m] << std::dec << std::endl;
                }
                //
        }
                ...
                const std::string& registerName)
{
        ...
        for (unsigned int s = 0; s < stringData.length(); s++)
                ...
                {
                        //std::cout << __COUT_HDR_FL__ << "S: " << s << " val: " << stringData.data()[s] << " b: " << b
<< " bit: " << ((stringData.data()[s]>>b)&1) << std::endl; data = 0x40 | (((stringData.data()[s] >> b) & 1) << 5);
...
//========================================================================================================================
bool FSSRPurdueFirmware::isClockStateExternal() //returns true if the clock state is External
{
...
//========================================================================================================================
void FSSRPurdueFirmware::setFrequencyRatio(std::string& buffer, int numerator, int denominator)
{
        //The device need to load numerator minus one and denominator minus one, with an internal address of 0x50 and
052 respectively OtsUDPFirmwareCore::write(buffer, STRIP_BCO_DCM, 0x80500000 + (numerator - 1)); //  Set BCOCLK
numerator // was 0x80500003
        //FIXME? Does Nothing RAR Aug 2017//OtsUDPFirmwareCore::waitClear(buffer, STRIP_BCO_DCM, 0xf0000000); //  Wait
DCM write to finish // was 0x80000000

        OtsUDPFirmwareCore::write(buffer, STRIP_BCO_DCM, 0x80520000 + (denominator - 1)); //  Set BCOCLK denominator //
was 0x80520001
        //FIXME? Does Nothing RAR Aug 2017//OtsUDPFirmwareCore::waitClear(buffer, STRIP_BCO_DCM, 0xf0000000); //  Wait
DCM write to finish - BCO frequency should be 13.513 MHz // was 0x80000000
}
...
std::string FSSRPurdueFirmware::resetSlaveBCO(void)
{
 ...
 //========================================================================================================================
 void FSSRPurdueFirmware::setBCO_0(uint32_t void FSSRPurdueFirmware::BCOOffset(uint32_t offset)
 {
