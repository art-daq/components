...
FSSRPurdueFirmware::FSSRPurdueFirmware(std::string  communicationFirmwareType,
    ...
    : FSSRFirmwareBase(communicationFirmwareType,
                       communicationFirmwareVersion,
...
//		communicationFirmwareInstance_  = new
// PurdueFirmwareCore(communicationFirmwareVersion); 	if(communicationFirmwareType ==
// FrontEndFirmwareBase::OTS_CORE_FIRMWARE_NAME) 		communicationFirmwareInstance_  =
// new OtsUDPFirmwareCore(communicationFirmwareVersion); 	else
//	{
...
//	//protocolInstance_ = new OtsFirmwareCore(version);//AUG-17-2017 RAR dissociated
// because function calls are entirely independent from PURDUE firmware calls //
////FrontEndFirmwareBase::getInstance(type,version);
...
//========================================================================================================================
void FSSRPurdueFirmware::init(void) {}
/*
//========================================================================================================================
std::string FSSRPurdueFirmware::configureClocks(std::string source, double frequency)
{
...
ethio stuff
    ...
    OtsUDPFirmwareCore::write(buffer, STRIP_CSR, stripCSRRegisterValue_); //  Reset CSR -
reset trigger counter, external 27 MHz clock

    resetDCMStripCSR(true);
    OtsUDPFirmwareCore::write(buffer, STRIP_CSR, stripCSRRegisterValue_); //  Set reset to
DCM

    resetDCMStripCSR(false);
    OtsUDPFirmwareCore::write(buffer, STRIP_CSR, stripCSRRegisterValue_); //  Clear reset
to DCM

    ;
    //FIXME? Does Nothing RAR Aug 2017// OtsUDPFirmwareCore::waitClear(buffer, STRIP_CSR,
waitDCMResetStripCSR()); //  Wait for DCM to lock

    OtsUDPFirmwareCore::write(buffer, STRIP_TRIM_CSR, 0x00002000); //  MCLKB edge for
channel 5 // was 0x00002000
    ...
    //FIXME? Does Nothing RAR Aug 2017//OtsUDPFirmwareCore::waitClear(buffer, STRIP_CSR,
waitDCMResetStripCSR()); //  Wait for DCM to lock std::cout << __COUT_HDR_FL__ <<
...
//========================================================================================================================
std::string FSSRPurdueFirmware::resetDetector(int channel)
{
    ...
    if (channel == -1)//reset all channels
    {
        ...
        OtsUDPFirmwareCore::write(buffer, STRIP_RESET, 0xf000003f); //  Issue reset // was
0xf000003f
        //FIXME? Does Nothing RAR Aug 2017//OtsUDPFirmwareCore::waitClear(buffer,
STRIP_RESET, 0xf0000000); //  Wait for reset to complete // was 0xf0000000 } else
    {
...
//========================================================================================================================
std::string FSSRPurdueFirmware::enableTrigger(void)
{
    std::string buffer;
...
NOTHING WORKS UNLESS THE BIT IS UNSET
    //setHaltStripCSR(0);//WARNING THIS IS CLEARED BY THE MASTER BUT IF THERE IS NO MASTER
NOTHING WORKS UNLESS THE BIT IS UNSET sendTriggerDataStripCSR(true);
    ...
    BCOOffset(4);
    OtsUDPFirmwareCore::write(buffer, STRIP_TRIG_CSR, stripTriggerCSRRegisterValue_); //
...
external trigger
    ...
    if (OtsUDPFirmwareCore::version_ == 1)
        OtsUDPFirmwareCore::write(buffer, STRIP_TRIG_INPUT_3, 0x20401000); // Turn on
streaming hits along with BCO data else if (OtsUDPFirmwareCore::version_ == 2)
        OtsUDPFirmwareCore::write(buffer, STRIP_TRIG_INPUT_3, 0x20301000); // Turn on
streaming hits along with BCO data else
    {
...
stripCSRRegisterValue_ << std::dec << std::endl; std::cout << __COUT_HDR_FL__ << "Done
...
//========================================================================================================================
void FSSRPurdueFirmware::readBurstDestinationIP(std::string& buffer)
{
    OtsUDPFirmwareCore::read(buffer, (uint64_t)0x0000000100000006);
}

//========================================================================================================================
void FSSRPurdueFirmware::readBurstDestinationMAC(std::string& buffer)
{
    OtsUDPFirmwareCore::read(buffer, (uint64_t)0x0000000100000007);
}

//========================================================================================================================
void FSSRPurdueFirmware::readBurstDestinationPort(std::string& buffer)
{
    OtsUDPFirmwareCore::read(buffer, (uint64_t)0x0000000100000008);
}

//========================================================================================================================
void FSSRPurdueFirmware::writeBurstDestinationIP(std::string& buffer, const uint64_t
value)
{
...
//========================================================================================================================
void FSSRPurdueFirmware::writeBurstDestinationMAC(std::string& buffer, const uint64_t
value)
{
...
//========================================================================================================================
void FSSRPurdueFirmware::writeBurstDestinationPort(std::string& buffer, const uint64_t
value)
{
...
//========================================================================================================================
std::string FSSRPurdueFirmware::resetBCO(void)
{
...
stripCSRRegisterValue_ << std::dec << std::endl;
    ...
    //write(buffer,STRIP_SC_CSR,0x900f0b95);//  This is the <SCR,set> command with the bit
set to sync SHIFT with BCO=0.
...
//========================================================================================================================
std::string FSSRPurdueFirmware::startStream(bool channel0, bool channel1, bool channel2,
...
0x0f000f30 OtsUDPFirmwareCore::write(buffer, STRIP_CSR, stripCSRRegisterValue_);
...
//========================================================================================================================
std::string FSSRPurdueFirmware::stopStream(void)
{
    std::string buffer;
...
//========================================================================================================================
void FSSRPurdueFirmware::makeDACSequence(FirmwareSequence<uint64_t>& sequence,
...
//========================================================================================================================
void FSSRPurdueFirmware::makeDACSequence(FirmwareSequence<uint32_t>& sequence,
        unsigned int channel, const ROCStream& rocStream)
{
    const ROCDACs& rocDACs = rocStream.getROCDACs();
    for (DACList::const_iterator it = rocDACs.getDACList().begin(); it
...
//========================================================================================================================
void FSSRPurdueFirmware::makeDACBuffer(std::string& buffer,
        unsigned int channel, const ROCStream& rocStream)
{
    std::cout << __COUT_HDR_FL__ << __PRETTY_FUNCTION__ << "Channel: " << channel <<
std::endl; const ROCDACs& rocDACs = rocStream.getROCDACs(); for (DACList::const_iterator
it = rocDACs.getDACList().begin(); it != rocDACs.getDACList().end(); it++)
    {
        std::string bufferElement;
        //FIXME? Does Nothing RAR Aug 2017//OtsUDPFirmwareCore::waitClear(bufferElement,
STRIP_SC_CSR, 0x80000000); uint32_t registerHeader = 0;
        //FIXME This must go in the FSSRROCDefinitions stuff
        if (it->first != "RejectHits" && it->first != "SendData")
        {
            OtsUDPFirmwareCore::write(bufferElement, ChannelFIFOAddress[channel],
it->second.second); registerHeader = FSSRROCDefinitions::makeDACWriteHeader(
                    rocStream.getFEWROCAddress(), it->first);
            //std::cout << __COUT_HDR_FL__ << __PRETTY_FUNCTION__ << "Register: " <<
it->first << " value: " << it->second.second << std::hex << " -> Data: " << registerHeader
<< std::dec << std::endl;
            //Insert channel
        ...
        //std::cout << __COUT_HDR_FL__ << __PRETTY_FUNCTION__ << "Register: " << it->first
<< " value: " << it->second.second << std::hex << " -> Data: " << registerHeader <<
std::dec << std::endl; OtsUDPFirmwareCore::write(bufferElement, STRIP_SC_CSR,
registerHeader);
        //FIXME? Does Nothing RAR Aug 2017//OtsUDPFirmwareCore::waitClear(bufferElement,
...
//========================================================================================================================
void FSSRPurdueFirmware:: makeDACBuffer(std::vector<std::string>& buffer, unsigned int
...
std::endl; buffer.push_back(bufferElement); limitCount = 0; bufferElement.clear();
        }
...
//========================================================================================================================
void FSSRPurdueFirmware::makeMaskBuffer(std::string& buffer, unsigned int channel,
...
//========================================================================================================================
void FSSRPurdueFirmware::makeMaskBuffer(std::string& buffer, unsigned int channel, const
...
"-------------------" << std::endl; for (unsigned int m = 0; m < 8 * 4; m++) { //m goes
from 0 to 31, since each byte has 8 bits, there are 32 bits val = mask[(8 * 4 * d) + m];
//assigns to val the value of the corresponding bit. 0-31, 32-63, 64-95, 96-127. it goes
through each of the 128 bits
            //std::cout << __COUT_HDR_FL__ << "---------------------" << j++ << std::endl;
            //std::cout << __COUT_HDR_FL__ << "data[" << d << "] before: " << std::hex <<
data[d] << std::dec << std::endl; data[d] |= (unsigned int) atoi(&val) << (8 * 4 - 1 - m);
            //std::cout << __COUT_HDR_FL__ << "(unsigned int) atoi(&val): " << (unsigned
int) atoi(&val) << std::endl;
            //std::cout << __COUT_HDR_FL__ << "data[" << d << "] after: " << std::hex <<
data[d] << std::dec << std::endl;
            //std::cout << __COUT_HDR_FL__ << std::hex << "D: " << data[d] << " Val: " <<
(unsigned int)atoi(&val) << " index: " << std::dec << (8*4-1-m) << " bit: " <<
...
//========================================================================================================================
void FSSRPurdueFirmware::makeMaskSequence(FirmwareSequence<uint64_t>& sequence,
        unsigned int channel, const ROCStream& rocStream)
{
    std::cout << __COUT_HDR_FL__ << "Making mask! " << std::endl;
...
//========================================================================================================================
void FSSRPurdueFirmware::makeMaskSequence(FirmwareSequence<uint32_t>& sequence,
        unsigned int channel, const ROCStream& rocStream)
{
    std::cout << __COUT_HDR_FL__ << "Making mask! " << std::endl;
    makeMaskSequence(sequence, channel, rocStream, "Kill");
...
clock the data in the fifo sequence.pushBack(ChannelFIFOAddress[channel], uInt64Data);
...
stringData.data()[s] << " b: " << b << " bit: " << ((stringData.data()[s]>>b)&1) <<
...
//========================================================================================================================
std::string FSSRPurdueFirmware::readCSRRegister()
{
    std::string buffer;
...
//========================================================================================================================
void FSSRPurdueFirmware::setFrequencyFromClockState(std::string& buffer, double frequency)
{
...
register
    //It will take two initial numerator and denominator bigger than necessary (to do not
loose precision)
...
//========================================================================================================================
bool FSSRPurdueFirmware::isClockStateExternal() //returns true if the clock state is
External
{
...
//========================================================================================================================
void FSSRPurdueFirmware::setCSRRegister(uint32_t total)
{
    stripCSRRegisterValue_ = total;
}

//========================================================================================================================
void FSSRPurdueFirmware::writeCSRRegister(std::string& buffer)
{
    OtsUDPFirmwareCore::write(buffer, STRIP_CSR, stripCSRRegisterValue_);
}

//========================================================================================================================
void FSSRPurdueFirmware::setPacketSizeStripCSR(uint32_t size)
{
    if (size > 31)
...
//========================================================================================================================
void FSSRPurdueFirmware::enableChannelsStripCSR(bool channel0, bool channel1,
...
//========================================================================================================================
void FSSRPurdueFirmware::setExternalBCOClockSourceStripCSR(std::string clockSource)
{
...
//========================================================================================================================
void FSSRPurdueFirmware::setHaltStripCSR(bool set)
{
    BitManipulator::insertBits(stripCSRRegisterValue_, (uint32_t) set, 17, 1);
}

//========================================================================================================================
void FSSRPurdueFirmware::enableBCOStripCSR(bool enable)
{
    BitManipulator::insertBits(stripCSRRegisterValue_, (uint32_t) enable, 19, 1);
}

//========================================================================================================================
void FSSRPurdueFirmware::flushBuffersStripCSR(void)
{
    BitManipulator::insertBits(stripCSRRegisterValue_, 1, 20, 1);
}

//========================================================================================================================
void FSSRPurdueFirmware::resetTriggerCounterStripCSR(std::string& buffer)
{
...
//========================================================================================================================
void FSSRPurdueFirmware::resetDAC(void)
{
    BitManipulator::insertBits(stripResetRegisterValue_, 1, 27, 1);
}

//========================================================================================================================
void FSSRPurdueFirmware::resetLink(bool channel0, bool channel1, bool channel2,
        bool channel3, bool channel4, bool channel5)
{
...
//========================================================================================================================
void FSSRPurdueFirmware::clearErrors(bool channel0, bool channel1, bool channel2,
        bool channel3, bool channel4, bool channel5)
{
    stripResetRegisterValue_ = 0;
    BitManipulator::insertBits(stripResetRegisterValue_,
...
//========================================================================================================================
void FSSRPurdueFirmware::clearFIFO(bool channel0, bool channel1, bool channel2,
        bool channel3, bool channel4, bool channel5)
{
    stripResetRegisterValue_ = 0;
    BitManipulator::insertBits(stripResetRegisterValue_,
...
//========================================================================================================================
void FSSRPurdueFirmware::resetChip(bool channel0, bool channel1, bool channel2,
        bool channel3, bool channel4, bool channel5)
{
    stripResetRegisterValue_ = 0;
    BitManipulator::insertBits(stripResetRegisterValue_,
...
//========================================================================================================================
void FSSRPurdueFirmware::setFrequencyRatio(std::string& buffer, int numerator, int
denominator)
{
    //The device need to load numerator minus one and denominator minus one, with an
internal address of 0x50 and 052 respectively OtsUDPFirmwareCore::write(buffer,
STRIP_BCO_DCM, 0x80500000 + (numerator - 1)); //  Set BCOCLK numerator // was 0x80500003
    //FIXME? Does Nothing RAR Aug 2017//OtsUDPFirmwareCore::waitClear(buffer,
STRIP_BCO_DCM, 0xf0000000); //  Wait DCM write to finish // was 0x80000000

    OtsUDPFirmwareCore::write(buffer, STRIP_BCO_DCM, 0x80520000 + (denominator - 1)); //
Set BCOCLK denominator // was 0x80520001
    //FIXME? Does Nothing RAR Aug 2017//OtsUDPFirmwareCore::waitClear(buffer,
STRIP_BCO_DCM, 0xf0000000); //  Wait DCM write to finish - BCO frequency should be 13.513
MHz // was 0x80000000
}

//========================================================================================================================
void FSSRPurdueFirmware::BCOOffset(uint32_t offset)
{
    BitManipulator::insertBits(stripTriggerCSRRegisterValue_, offset, 0, 4);
}

//========================================================================================================================
void FSSRPurdueFirmware::selectSpyFIFO(uint32_t input)
{
    BitManipulator::insertBits(stripTriggerCSRRegisterValue_, input, 4, 3);
}

//========================================================================================================================
void FSSRPurdueFirmware::halt(bool halt)
{
    BitManipulator::insertBits(stripTriggerCSRRegisterValue_, (uint32_t) halt, 7, 1);
}

//========================================================================================================================
void FSSRPurdueFirmware::configureStripTriggerUnbiased(std::string& buffer)
{
...
//========================================================================================================================
void FSSRPurdueFirmware::configureTriggerInputs(std::string& buffer)
{
    OtsUDPFirmwareCore::write(buffer, STRIP_TRIG_INPUT_0, 0x3f440000); //  FSSR GOTHIT
trigger input channel 0,1 OtsUDPFirmwareCore::write(buffer, STRIP_TRIG_INPUT_1,
0x3f440000); //  FSSR GOTHIT trigger input channel 2,3 OtsUDPFirmwareCore::write(buffer,
STRIP_TRIG_INPUT_2, 0x0); //  FSSR GOTHIT trigger input channel 4,5
}

//========================================================================================================================
std::string FSSRPurdueFirmware::resetSlaveBCO(void)
{
    std::string buffer;
 ...
 //========================================================================================================================
 void FSSRPurdueFirmware::chipID(uint32_t size)
 {
 BitManipulator::insertBits(stripSCRegisterValue_, size, 0, 5);
 }

 //========================================================================================================================
 void FSSRPurdueFirmware::addressSlowControls(uint32_t size)
 {
 BitManipulator::insertBits(stripSCRegisterValue_, size, 5, 5);
 }

 //========================================================================================================================
 void FSSRPurdueFirmware::instructionSlowControls(uint32_t size)
 {
 BitManipulator::insertBits(stripSCRegisterValue_, size, 10, 3);
 }

 //========================================================================================================================
 void FSSRPurdueFirmware::channelreadSelect(uint32_t size)
 {
 BitManipulator::insertBits(stripSCRegisterValue_, size, 13, 3);
 }

 //========================================================================================================================
 void FSSRPurdueFirmware::channelMask(uint32_t size)
 {
 BitManipulator::insertBits(stripSCRegisterValue_, size, 16, 8);
 }

 //========================================================================================================================
 void FSSRPurdueFirmware::bitsLength(uint32_t length)
 {
