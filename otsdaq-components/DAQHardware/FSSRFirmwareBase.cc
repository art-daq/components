...
FSSRFirmwareBase::FSSRFirmwareBase(const std::string& communicationFirmwareType,
                                   unsigned int communicationFirmwareVersion, unsigned int version)
  ...
  if (communicationFirmwareType ==
      FSSRFirmwareBase::PURDUE_FIRMWARE_NAME)  // could also use string and: if(choice == "purdue") etc.
    communicationFirmwareInstance_ = new PurdueFirmwareCore(communicationFirmwareVersion);
  else if (communicationFirmwareType == FSSRFirmwareBase::OTS_FIRMWARE_NAME)  // AUG-17-2017 RAR dissociated because
                                                                              // function calls are entirely independent
                                                                              // from PURDUE firmware calls //
    communicationFirmwareInstance_ = new OtsUDPFirmwareCore(communicationFirmwareVersion);
  else {
    __SS__ << "Unknown communication firmware type choice: " << communicationFirmwareType << std::endl;
    __COUT_ERR__ << ss.str();
    __SS_THROW__;
  }
...
std::string FSSRFirmwareBase::universalRead(char* address) {
  __COUT__ << "universalRead communicationFirmwareType_ " << communicationFirmwareType_ << std::endl;
  return communicationFirmwareInstance_->read(address);
}

//========================================================================================================================
std::string FSSRFirmwareBase::universalWrite(char* address, char* data) {
  __COUT__ << "universalWrite communicationFirmwareType_ " << communicationFirmwareType_ << std::endl;
...
std::string FSSRFirmwareBase::configureClocks(std::string source, double frequency) {
  ...
  communicationFirmwareInstance_->write(
      buffer, STRIP_CSR, stripCSRRegisterValue_);  //  Reset CSR - reset trigger counter, external 27 MHz clock
...
void FSSRFirmwareBase::resetDCM(std::string& buffer) {
  ...
  // communicationFirmwareInstance_->waitClear(buffer, STRIP_CSR, waitDCMResetStripCSR(),false/*clearBuffer*/); //  Wait
  // for DCM to lock
}
...
void FSSRFirmwareBase::alignReadOut(std::string& buffer, unsigned int sensor, unsigned int chip) {
  ...
  communicationFirmwareInstance_->write(
      buffer, STRIP_TRIM_CSR,
      (0x9 << 28) + ((chip & 0xf) << 3) + (sensor & 0x7));  //  MCLKB edge for channel 5 // was 0x00002000
}
//========================================================================================================================
std::string FSSRFirmwareBase::resetDetector(int channel) {
  ...
  // 15-8  unused
  // Byte 1-------------------
  // 7-0   CHANNEL_MASK ->Reset signals are sent to all channels with bits set in CHANNEL_MASK field
  ...
  if (channel == -1)  // reset all channels
  {
    ...
    // 2018-10-24 Lorenzo in OtsUDPFirmware is doing nothing so I am commenting it out
    // communicationFirmwareInstance_->waitClear(buffer, STRIP_RESET, 0xf0000000); //  Wait for reset to complete // was
    // 0xf0000000
  } else {
...
std::string FSSRFirmwareBase::enableTrigger(void) {
  ...
  BCOOffset(4);
  communicationFirmwareInstance_->write(buffer, STRIP_TRIG_CSR, stripTriggerCSRRegisterValue_,
                                        false /*clearBuffer*/);  //  BCO offset // was 0x00000004

  //  write(buffer,STRIP_TRIG_INPUT_0,0x1f060040);  //  FSSR GOTHIT trigger input - timed in for the 27 MHz external
  //  clock write(buffer,STRIP_TRIG_INPUT_3,0x3f874040);  //  Unbiased trigger input + external trigger
  ...
  communicationFirmwareInstance_->write(buffer, STRIP_TRIG_INPUT_3, 0x20401000, false /*clearBuffer*/);
  // else if (communicationFirmwareInstance_->getVersion() == 2)
  //	communicationFirmwareInstance_->write(buffer, STRIP_TRIG_INPUT_3, 0x20301000, false/*clearBuffer*/);
  // else
  //{
...
//    std::cout << __COUT_HDR_FL__ << "ADDRESS: " << std::hex << ntohl(socketAddress.sin_addr.s_addr) << std::dec <<
//    std::endl; communicationFirmwareInstance_->write(buffer, DATA_DESTINATION_IP,
//    ntohl(socketAddress.sin_addr.s_addr)); //  Set data destination IP 192.168.133.1 std::cout << __COUT_HDR_FL__ <<
//    "PORT: " << std::hex << port << std::dec << std::endl; communicationFirmwareInstance_->write(buffer,
//    DATA_SOURCE_DESTINATION_PORT, port); //  Set data destination port std::cout << __COUT_HDR_FL__ << "THIS IS THE
...
std::string FSSRFirmwareBase::resetBCO(void) {
  ...
  // std::cout << __COUT_HDR_PL__ << "stripCSRRegisterValue :" << std::hex << stripCSRRegisterValue_ << std::dec <<
  // std::endl;
  ...
  // RAW          0
  // SC_BUSY      1
  // When BCO SYNC is set the transaction is delayed until the internal BCO counter has a value of 240. This allow the
  // AqBCO command to be synchronized
  communicationFirmwareInstance_->write(
      buffer, STRIP_SC_CSR, 0x903f0b95,
      false /*clearBuffer*/);  //  This is the <SCR,set> command with the bit set to sync SHIFT with BCO=0.

  // enableBCOStripCSR(true);
  // communicationFirmwareInstance_->write(buffer, STRIP_CSR, stripCSRRegisterValue_);

  std::cout << __COUT_HDR_PL__ << "stripCSRRegisterValue out:" << std::hex << stripCSRRegisterValue_ << std::dec
            << std::endl;
...
//========================================================================================================================
std::string FSSRFirmwareBase::startStream(bool channel0, bool channel1, bool channel2, bool channel3, bool channel4,
                                          bool channel5) {
  //	std::cout << __COUT_HDR_FL__ << "Start Stream!" << std::endl;
  //	std::cout << __COUT_HDR_PL__ << "stripCSRRegisterValue in:" << std::hex << stripCSRRegisterValue_ << std::dec <<
  //std::endl;
  std::string buffer;

  //	std::cout << __COUT_HDR_PL__ << " channel0 " << channel0 << " channel1 " << channel1 << " channel2 " << channel2
  ...
  //	std::cout << __COUT_HDR_PL__ << "stripCSRRegisterValue out:" << std::hex << stripCSRRegisterValue_ << std::dec
  //<< std::endl; 	std::cout << __COUT_HDR_PL__ << "Done start Stream!" << std::endl;
...
//========================================================================================================================
void FSSRFirmwareBase::makeDACSequence(FirmwareSequence<uint64_t>& sequence, unsigned int channel,
  ...
  for (DACList::const_iterator it = rocDACs.getDACList().begin(); it != rocDACs.getDACList().end(); it++) {
    ...
    sequence.pushBack(ChannelFIFOAddress[channel], BitManipulator::insertBits(data, (uint64_t)0x48, 56, 8));
    // set WRITE
    sequence.pushBack(ChannelFIFOAddress[channel], BitManipulator::insertBits(data, (uint64_t)1, 60, 1));
    // clr WRITE
    sequence.pushBack(ChannelFIFOAddress[channel], BitManipulator::insertBits(data, (uint64_t)0, 60, 1));
    // clr TALK
    sequence.pushBack(ChannelFIFOAddress[channel], BitManipulator::insertBits(data, (uint64_t)0, 62, 1));
    sequence.pushBack(ChannelFIFOAddress[channel], BitManipulator::insertBits(data, (uint64_t)0x40, 56, 8));
    // break;
  }
}

//========================================================================================================================
void FSSRFirmwareBase::makeDACSequence(FirmwareSequence<uint32_t>& sequence, unsigned int channel,
                                       const ROCStream& rocStream) {
  const ROCDACs& rocDACs = rocStream.getROCDACs();
  for (DACList::const_iterator it = rocDACs.getDACList().begin(); it != rocDACs.getDACList().end(); it++) {
    /*RYAN
...
sequence.pushBack(ChannelFIFOAddress[channel], BitManipulator::insertBits(data,(uint32_t)0x40,56,8));
    ...
    // if(it->first != "SendData" && it->first != "RejectHits") continue;
    uint32_t data = FSSRROCDefinitions::makeDACWriteHeader(rocStream.getFEWROCAddress(), it->first);
    // Insert channel
    BitManipulator::insertBits(data, 1, 16 + channel, 1);
    sequence.pushBack(ChannelFIFOAddress[channel], it->second.second);
    std::cout << __COUT_HDR_FL__ << "Register: " << it->first << " value: " << it->second.second << std::hex
...
void FSSRFirmwareBase::makeDACBuffer(std::string& buffer, unsigned int channel, const ROCStream& rocStream) {
  ...
  //		if (it->first == "VTp0")
  //		{
  //			//communicationFirmwareInstance_->write(bufferElement, ChannelFIFOAddress[channel],
  ...
  ////			{
  ////				registerHeader = FSSRROCDefinitions::makeDACSetHeader(rocStream.getFEWROCAddress(),
  ///it->first); /				//Insert channel /
  ///BitManipulator::insertBits(registerHeader, 1, 16 + channel, 1); /			} /			else if
  ///(it->second.second == 0 || it->second.second == 5) /			{ /
  ...
  //
  //	    std::cout << __COUT_HDR_FL__ << "Register: " << it->first << " value: " << it->second.second << std::hex <<
  //" -> Data: " << registerHeader << std::dec << std::endl;
  ...
  for (DACList::const_iterator it = rocDACs.getDACList().begin(); it != rocDACs.getDACList().end(); it++) {
    std::string bufferElement;
    communicationFirmwareInstance_->waitClear(bufferElement, STRIP_SC_CSR, 0x80000000, false);
    uint32_t registerHeader = 0;
    // FIXME This must go in the FSSRROCDefinitions stuff
    if (it->first != "RejectHits" && it->first != "SendData") {
      communicationFirmwareInstance_->write(bufferElement, ChannelFIFOAddress[channel], it->second.second, false);
      ...
      if (it->second.second == 1 || it->second.second == 2) {
        ...
        std::cout << __COUT_HDR_FL__ << "Register value for : " << it->first
                  << " doesn't have a value I expect -> value = " << it->second.second << std::endl;
    }

    // std::cout << __COUT_HDR_FL__ << "Register: " << it->first << " value: " << it->second.second << std::hex << " ->
    // Data: " << registerHeader << std::dec << std::endl;
    communicationFirmwareInstance_->write(bufferElement, STRIP_SC_CSR, registerHeader, false);
    communicationFirmwareInstance_->waitClear(bufferElement, STRIP_SC_CSR, 0x80000000, false);
...
//========================================================================================================================
void FSSRFirmwareBase::makeDACBuffer(std::vector<std::string>& buffer, unsigned int channel,
                                     const ROCStream& rocStream) {
  ...
  //	for (std::map<std::string, std::pair<unsigned int, unsigned int> >::const_iterator it =
  //rocDACs.getDACList().begin(); it != rocDACs.getDACList().end(); it++)
  {
    // std::cout << __COUT_HDR_FL__ << "Register?" << std::endl;
    //		if (it.first != "ActiveLines") continue;
    //		std::cout << __COUT_HDR_FL__ << "Register-1: " << it->first << " val: " << it->second.first  << " = " <<
    //it->second.second << std::endl;
    communicationFirmwareInstance_->waitClear(bufferElement, STRIP_SC_CSR, 0x80000000, false);
    uint32_t registerHeader = 0;
    // FIXME This must go in the FSSRROCDefinitions stuff
    ...
    if (it.first != "RejectHits" && it.first != "SendData") {
      // std::cout << __COUT_HDR_FL__ << "Register1: " << it.first << "Channel: " << channel << " fifo address: " <<
      // ChannelFIFOAddress[channel] << std::endl;
      communicationFirmwareInstance_->write(bufferElement, ChannelFIFOAddress[channel], it.second.second, false);
      registerHeader = FSSRROCDefinitions::makeDACWriteHeader(rocStream.getFEWROCAddress(), it.first);
      // Insert channel
      BitManipulator::insertBits(registerHeader, 1, 16 + channel, 1);
...
//========================================================================================================================
void FSSRFirmwareBase::makeMaskBuffer(std::string& buffer, unsigned int channel, const ROCStream& rocStream) {
...
void FSSRFirmwareBase::makeMaskBuffer(std::string& buffer, unsigned int channel, const ROCStream& rocStream,
  ...
  char val;
  // int j = 0;
  for (unsigned int d = 0; d < 4; d++) {  // d goes from 0 to 4. 4 bytes
    // std::cout << __COUT_HDR_FL__ << "---------------------" << d << "-------------------" << std::endl;
    for (unsigned int m = 0; m < 8 * 4; m++) {  // m goes from 0 to 31, since each byte has 8 bits, there are 32 bits
      val = mask[(8 * 4 * d) + m];  // assigns to val the value of the corresponding bit. 0-31, 32-63, 64-95, 96-127. it
                                    // goes through each of the 128 bits
      ...
      // std::cout << __COUT_HDR_FL__ << std::hex << "D: " << data[d] << " Val: " << (unsigned int)atoi(&val) << "
      // index: " << std::dec << (8*4-1-m) << " bit: " << mask[(8*4*d)+m] << std::dec << std::endl;
    }
    //
  }
  ...
  for (i = 0; i < 4; i++)
    ...
    communicationFirmwareInstance_->write(buffer, STRIP_SCI + 4 * (4 - i - 1), data[i], false);
...
//========================================================================================================================
void FSSRFirmwareBase::makeMaskSequence(FirmwareSequence<uint64_t>& sequence, unsigned int channel,
                                        const ROCStream& rocStream) {
  std::cout << __COUT_HDR_FL__ << "Making mask! " << std::endl;
...
//========================================================================================================================
void FSSRFirmwareBase::makeMaskSequence(FirmwareSequence<uint32_t>& sequence, unsigned int channel,
                                        const ROCStream& rocStream) {
  std::cout << __COUT_HDR_FL__ << "Making mask! " << std::endl;
  makeMaskSequence(sequence, channel, rocStream, "Kill");
...
//========================================================================================================================
void FSSRFirmwareBase::makeMaskSequence(FirmwareSequence<uint64_t>& sequence, unsigned int channel,
                                        const ROCStream& rocStream, const std::string& registerName) {
  ...
  for (unsigned int s = 0; s < stringData.length(); s++)
    for (int b = 8 - 1; b >= 0 && (s * 8 + 8 - b < 13 + 128); b--) {
      // std::cout << __COUT_HDR_FL__ << "S: " << s << " val: " << stringData.data()[s] << " b: " << b << " bit: " <<
...
void FSSRFirmwareBase::setPacketSizeStripCSR(uint32_t size) {
  if (size > 31)
    std::cout << __COUT_HDR_FL__ << "ERROR: Maximum packet size is 31 while here you are trying to set " << size
              << " packets!" << std::endl;
...
//========================================================================================================================
void FSSRFirmwareBase::enableChannelsStripCSR(bool channel0, bool channel1, bool channel2, bool channel3, bool channel4,
                                              bool channel5) {
  BitManipulator::insertBits(stripCSRRegisterValue_,
                             ((uint32_t)channel0) + ((uint32_t)channel1 << 1) + ((uint32_t)channel2 << 2) +
                                 ((uint32_t)channel3 << 3) + ((uint32_t)channel4 << 4) + ((uint32_t)channel5 << 5),
                             8, 6);
}
...
//========================================================================================================================
void FSSRFirmwareBase::armBCOResetCSR(void) { BitManipulator::insertBits(stripCSRRegisterValue_, 1, 19, 1); }

//========================================================================================================================
void FSSRFirmwareBase::flushBuffersStripCSR(void) { BitManipulator::insertBits(stripCSRRegisterValue_, 1, 20, 1); }
...
//========================================================================================================================
void FSSRFirmwareBase::resetBCOCounterStripCSR(void) { BitManipulator::insertBits(stripCSRRegisterValue_, 1, 22, 1); }
...
//========================================================================================================================
void FSSRFirmwareBase::resetDAC(void) { BitManipulator::insertBits(stripResetRegisterValue_, 1, 27, 1); }

//========================================================================================================================
void FSSRFirmwareBase::resetLink(bool channel0, bool channel1, bool channel2, bool channel3, bool channel4,
                                 bool channel5) {
...
//========================================================================================================================
void FSSRFirmwareBase::clearErrors(bool channel0, bool channel1, bool channel2, bool channel3, bool channel4,
                                   bool channel5) {
  stripResetRegisterValue_ = 0;
  BitManipulator::insertBits(stripResetRegisterValue_,
                             ((uint32_t)channel0) + ((uint32_t)channel1 << 1) + ((uint32_t)channel2 << 2) +
...
//========================================================================================================================
void FSSRFirmwareBase::clearFIFO(bool channel0, bool channel1, bool channel2, bool channel3, bool channel4,
                                 bool channel5) {
  stripResetRegisterValue_ = 0;
  BitManipulator::insertBits(stripResetRegisterValue_,
                             ((uint32_t)channel0) + ((uint32_t)channel1 << 1) + ((uint32_t)channel2 << 2) +
...
//========================================================================================================================
void FSSRFirmwareBase::resetChip(bool channel0, bool channel1, bool channel2, bool channel3, bool channel4,
                                 bool channel5) {
  stripResetRegisterValue_ = 0;
  BitManipulator::insertBits(stripResetRegisterValue_,
                             ((uint32_t)channel0) + ((uint32_t)channel1 << 1) + ((uint32_t)channel2 << 2) +
...
void FSSRFirmwareBase::setFrequencyRatio(std::string& buffer, int numerator, int denominator) {
  ...
  // respectively
  communicationFirmwareInstance_->write(buffer, STRIP_BCO_DCM,
                                        0x80500000 + (numerator - 1));  //  Set BCOCLK numerator // was 0x80500003
  communicationFirmwareInstance_->waitClear(buffer, STRIP_BCO_DCM,
  ...
  communicationFirmwareInstance_->waitClear(
      buffer, STRIP_BCO_DCM,
      0xf0000000);  //  Wait DCM write to finish - BCO frequency should be 13.513 MHz // was 0x80000000
}
...
void FSSRFirmwareBase::configureStripTriggerUnbiased(std::string& buffer) {
  communicationFirmwareInstance_->write(buffer, STRIP_TRIG_UNBIASED, 0x002805c,
                                        false /*clearBuffer*/);  //  Configure unbiased trigger
}

//========================================================================================================================
void FSSRFirmwareBase::configureTriggerInputs(std::string& buffer) {
  //	communicationFirmwareInstance_->write(buffer, STRIP_TRIG_INPUT_0, 0x3f440000); //  FSSR GOTHIT trigger input
  //channel 0,1 	communicationFirmwareInstance_->write(buffer, STRIP_TRIG_INPUT_1, 0x3f440000); //  FSSR GOTHIT
  //trigger input channel 2,3
  communicationFirmwareInstance_->write(buffer, STRIP_TRIG_INPUT_0, 0x0,
                                        false /*clearBuffer*/);  //  FSSR GOTHIT trigger input channel 0,1
  communicationFirmwareInstance_->write(buffer, STRIP_TRIG_INPUT_1, 0x0,
                                        false /*clearBuffer*/);  //  FSSR GOTHIT trigger input channel 2,3
  communicationFirmwareInstance_->write(buffer, STRIP_TRIG_INPUT_2, 0x0,
