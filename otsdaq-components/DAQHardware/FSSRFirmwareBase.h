...
//		- FrontEndFirmwareBase (all virtual read, write, setDest, init) *functions are not pure virtual, rather
//throw exceptions if not overridden to allow setDest to go unused, for example.
...
//
//		- FSSRFirmwareBase (takes ONLY parameter for communication type. Does not inherit from anything).
//*Implements 90-100% base fssr functionality:
//		- FSSROtsFirmware (inherits from FSSRApplicationFirmwareBase only and overrides any 10% special
//functions)
//		- FSSRPurdueFirmware (inherits from FSSRApplicationFirmwareBase only and overrides any 10% special
//functions) 					FSSRFirmwareBase::setCSR()
//					{
//					Communication->write(CSR);
//					}
//
//		- At PLUGIN level (takes parameter for communication AND application firmware type.  ALSO -PRESTON WAS
//partially RIGHT - inherit from FrontEndFirmwareBase AND FSSRFirmwareBase. ) * implement all the base functions using
//the pointers:
//
...
//					}
//
//		* This way the FE interface plugin code is exactly the same for both cases, just modified by type
//strings.
//		* Since only modified by type strings, this means there only needs to be one FE plugin class! Types
...
class FSSRFirmwareBase {
 public:
  FSSRFirmwareBase(const std::string& communicationFirmwareType, unsigned int communicationFirmwareVersion = -1,
  ...
  // virtual void  		    setDataDestination              (std::string& buffer, const std::string& ip, const uint16_t
  // port);//					{__SS__; __THROW__(ss.str() + "Illegal call to undefined base class member function");
  // return;};
  ...
  // void 					setHaltStripCSR                  (bool set);//2018-10-24 Doesn't
  // exist????
